...
.PHONY: shfmt
shfmt: .cross.sh .lintsetup.sh
	@command -v shfmt > /dev/null 2>&1 || \
		{ printf '%s\n' "⚠️ shfmt not found!"; exit 0; } ; \
		set -x; shfmt -bn -sr -fn -i 2 -s -d .cross.sh .lintsetup.sh

##############################################################################
# Target: shellcheck

.PHONY: shellcheck
shellcheck: .cross.sh .lintsetup.sh
	@command -v shellcheck > /dev/null 2>&1 || \
		{ printf '%s\n' "⚠️ shellcheck not found!"; exit 0; } ; \
		set -x; shellcheck -s sh -o any,all .cross.sh .lintsetup.sh
